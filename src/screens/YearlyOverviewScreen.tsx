...
export default function YearlyOverviewScreen() {
  ...
  filtered.forEach((entry) => {
    entry.metrics.forEach((m) => {
      ...
      statsMap[key] =
        (statsMap[key] || 0) + parseFloat(m.value.replace(/\s|,/g, '').replace(',', '.'));
  ...
  const rows = [
    { label: 'Total PnL (*1/1)', key: 'totalPL' },
    { label: 'Total Trades (*1/1)', key: 'totalTrades' },
    { label: 'Total Profit (*1/1)', key: 'profit' },
    { label: 'Total Loss (*1/1)', key: 'loss' },
